...
# Name-cleaning helpers live in their own files so that wrapper scripts can
# reuse them. They are sourced (not executed) because they return their results
# in memory via shell globals / namerefs rather than via stdout or temp files.
source "$scriptDir/cleanNamesIndividually.sh"

source "$scriptDir/cleanNamesCollectively.sh"
